	...
	if(H.mind)
		...
		switch(fixation_choice)
			if("Of Arcyne Laws and Convolutions") //you get better magic than the other tinkerers, but still on the lower end of magic users who get a major aspect.
				to_chat(H, span_warning("Within the secrets of the Arcyne lie the fundement, the true underpinnings of reality, dream, and divinity. Through Arcyne mastery, the world might be brought to heel. No pursuit is greater"))
